...
class ErrorBoundary extends React.Component {
  constructor(props) {
    super(props)
    this.state = { hasError: false, githubStars: null }
  }
  ...
  async fetchRepoData() {
    try {
      const response = await fetch('https://api.github.com/repos/gofr-dev/gofr')
      if (!response.ok) {
    ...
    if (this.state.hasError) {
      return (
        <div className={``}>
          <header
            className={clsx(
              'sticky top-0 z-50 flex flex-none flex-wrap items-center justify-between bg-white px-4 py-5 shadow-md shadow-slate-900/5 transition duration-500 dark:bg-transparent dark:shadow-none sm:px-6 lg:px-8',
            )}
          >
          ...
          <div className={`flex h-[70vh] items-center justify-center`}>
            <div
              className={`flex flex-col items-center gap-4 text-xl md:text-3xl font-semibold text-center`}
            >
              <h2>
                <span className={`text-sky-400`}>Oops</span>,{' '}
